        ...
        if stream['name'] == pa['stream'] && stream['app'] = pa['app'] && stream['publish']['active']
          ...
          when 6000..10000 # 1080p@60fps -> 720p, 480p, 360p
            ['720p'].each do |transcode_stream|
              ...
              transcodes << {transcode_stream: transcode_stream}
            end
          ...
          transcodes.each do |t|
            ...
            job = TranscodeJob.perform_later(t[:transcode_stream], input_rtmp, "#{output_rtmp_prefix}_#{t[:transcode_stream]}", transcode.id)
            transcode.update(job_id: job.provider_job_id)
          end
        end
